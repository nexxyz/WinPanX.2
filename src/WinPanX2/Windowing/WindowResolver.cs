...
internal static class WindowResolver
{
    ...
    public static WindowInfo? ResolveForProcess(int pid, bool preferForeground)
    {
        ...
        void AddCandidates(IEnumerable<WindowInfo> list, CandidateSource source, int parentDepth = 0)
        {
        ...
        AddCandidates(windows.Where(w => w.ProcessId == pid), CandidateSource.DirectPid);

        // 2) Walk parent chain and include those windows too (Chromium audio often lives in child processes).
        var current = pid;
        for (int depth = 1; depth <= 10; depth++)
        {
            var parent = ProcessHelper.GetParentProcessId(current);
            ...
            AddCandidates(windows.Where(w => w.ProcessId == parent.Value), CandidateSource.ParentPid, depth);
        ...
        {
            AddCandidates(
                windows.Where(w => ProcessHelper.GetProcessName(w.ProcessId)?.Equals(exe, StringComparison.OrdinalIgnoreCase) == true),
                CandidateSource.ExeMatch);
        }
        ...
        {
            var score = ScoreCandidate(c, fg, preferForeground, out _);
            if (score > bestScore)
            {
